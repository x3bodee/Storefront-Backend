...
import {
  convert_ids_to_sql_list,
  check_product_list,
} from '../helper/helper_for_product_model';
...
export class OrderModel {
  ...
  async create(
    ...
    try {
      ...
      console.log(
        'check_product_list: ',
        check_product_list(list, converted_id_list)
      );
      if (!check_product_list(list, converted_id_list))
      ...
      const order_product = products_list.map((e) =>
        e.concat(insert_order_result.rows[0].order_id)
      );
      console.log(order_product);
      const insert_order_products_sql =
        'insert into order_product (product_id, order_quantity, order_id) values %L';
      const insert_order_products_result = await conn.query(
        format(insert_order_products_sql, order_product)
      );
