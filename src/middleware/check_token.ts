...
export function check_token(req:Request, res:Response, next:NextFunction){
    ...
    try {
        ...
        if( error.includes('Token is required!!') ) return res.status(401).json({status:false, msg:"Token is required", err:"ERROR: Token is required"}); 
        if (err instanceof jwt.JsonWebTokenError ) return res.status(401).json({status:false, msg:"Invalid Token", err:"ERROR: Invalid JWT Token"});
        if( error.includes('Invalid Token') ) return res.status(401).json({status:false, msg:"Invalid Token", err:"ERROR: Invalid Token"}); 
        throw new Error(`${err}`);
